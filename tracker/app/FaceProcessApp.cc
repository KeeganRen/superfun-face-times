...
void FaceProcessApp::align(cv::Mat &shape,cv::Mat &visi){
    ...
    for(i = 0; i < facePoints.size(); i++){
        circle(frame, facePoints[i], 4, CV_RGB(255, 0, 0), 2, 8, 0);
	}
    imshow("face pts", frame);
    cvWaitKey(0);
...
//	imshow("orig image", frame);
    ...
    cv::Mat homography = cv::findHomography(facePoints, canonicalPoints);
 
    cv::Mat warped2 = cv::Mat::zeros(500, 500, frame.type() );
    cv::warpPerspective(frame, warped2, homography, warped2.size() );
    
    imshow("warped - homography", warped2);
    cvWaitKey(0);
